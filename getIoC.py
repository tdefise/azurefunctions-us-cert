...
def parse_STIX(stix_file_filename, title):
    with open(stix_file_filename, 'r') as file:
        data = file.read()
...
def post_cti_to_microsoft_graph():
    
    headers = {'content-type': 'application/json'}
    
    r = requests.post('https://graph.microsoft.com/beta/security/tiIndicators', json={"key": "value"})
    >>> r.status_code
    200
        ...
        for entry in feed.entries:
            ...
            if stix_file_url is not None:
                ...
                if not check_file_already_in_az_sa(stix_file_filename,az_blob_conn_string,az_sa_container):
                    ...
                    print(parsed_observables)
                
                """
                parse_STIX(stix_file_url,title)
                """
    except Exception as error:
        print("Unexpected Error : ", error)            
